...
function checkScroll() {
  const headerImageHeight = document.querySelector('.header-image').offsetHeight;
  const upperFooterSection = document.querySelector('.upperfooter-section');
  const scrollToFooterHeight = upperFooterSection.offsetTop - upperFooterSection.offsetHeight - 690;
  const scrollTop = window.pageYOffset || document.documentElement.scrollTop;

  const fixedBlogs = document.querySelector('.fixed-blogs');

  if (scrollTop >= headerImageHeight - 90 && scrollTop < scrollToFooterHeight) {
      fixedBlogs.style.position = 'fixed';
      fixedBlogs.style.top = '155px';
      fixedBlogs.style.bottom = 'auto';
  } else if (scrollTop > scrollToFooterHeight) {
      fixedBlogs.style.position = 'absolute';
      fixedBlogs.style.bottom = '0px';
      fixedBlogs.style.top = 'auto';
  } else if (scrollTop <= headerImageHeight + 80) {
      fixedBlogs.style.position = 'static';
  }
}
...
function checkWindowSize() {
  if (window.innerWidth >= 991) {
      ...
      document.querySelector('.fixed-blogs').style.position = 'static';
  }
}
